...
var root = {
  ...
  setrestaurant: ({ input }) => {
    // Your code goes here
    const newId = restaurants[restaurants.length - 1].id + 1;
    restaurants.push({
      id: newId,
      ...input,
    });
  ...
  editrestaurant: ({ id, ...restaurant }) => {
    // Your code goes here
    const record = restaurants.filter((r) => r.id === id)[0];
    if (!record) throw new Error("restaurant not found");
  ...
  deleterestaurant: ({ id }) => {
    ...
    return { ok: restaurants.length < len };
  },
};
var app = express();
app.use(
